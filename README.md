...
<p align="center">
  <b>v4.7.20</b> &middot; MIT &middot; macOS + Linux
</p>
...
Codex uses an MCP server (`mcp/apd_mcp_server.py`) that exposes the pipeline as 6 MCP tools: `apd_ping`, `apd_doctor`, `apd_advance_pipeline`, `apd_guard_write`, `apd_verify_step`, `apd_adversarial_pass`. The orchestrator on Codex plays all roles inline — there is no sub-agent dispatch like on CC — and scope enforcement happens through `apd_guard_write` instead of per-agent hook files.

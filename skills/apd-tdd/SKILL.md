...
- Then dispatch the regular `code-reviewer` and run `pipeline-advance reviewer` — that step writes `.adversarial-pending` as the green light for adversarial dispatch. It writes the marker only when an `adversarial-reviewer` definition exists; missing definition and no valid `adversarial: skip` → the advance BLOCKs (`adversarial-agent-missing`) rather than dropping the layer

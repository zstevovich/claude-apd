...
5. **supervision** (v6.30, CC-owned; also active in Codex sessions of hybrid projects sharing the config) — when the declared `MODEL_PROFILE` has a `supervisor` row (v1: eco) + Full mode: supervisor dispatch over the FINAL diff → `.supervision-summary` (+ `.supervision-rationale.md` if T>0). Verifier gate checks it (default warn — Phase 1)
...
| File | Format | Notes |
|---|---|---|
| `pipeline-metrics.log` | Pipe-delimited per-task: `ts\|task\|spec_ts\|builder_ts\|reviewer_ts\|verifier_ts\|status\|adv_t\|adv_a\|adv_d\|agents_t\|agents_x\|adv_do\|adv_dr\|adv_w\|sup_t\|sup_a\|sup_d` | Skips `APD-VERIFY-*` synthetic test runs. **v6.7.1:** columns 13–15 added (`adv_do` = orchestrator dismissals, `adv_dr` = reviewer-self-dismissals, `adv_w` = soft-warn count from rationale-quality scan). `adv_d` in column 10 stays as `adv_do + adv_dr` sum for backward compat with v6.6 and earlier readers. **v6.30:** columns 16–18 (`sup_t\|sup_a\|sup_d` from `.supervision-summary`; zeros when supervision didn't run). `apd report` names every column with a `_rest` sink (this also fixed a pre-existing v6.7.1 reader bug: columns 13+ were absorbed into `agents_x`, silently suppressing the MaxTurn line on 15-col rows); rows with `sup_t>0` render a `Supervision:` line |

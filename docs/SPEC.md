...
| `guard-secrets` | PreToolUse `Read\|Write\|Edit\|Bash` (session-level since v6.38 — it was frontmatter-only, i.e. never executed on CC; see §7.4) | Customizable pattern list (default: `.env.production`, `.env.staging`, `.pem`, `.key`, `id_rsa`, `id_ed25519`, `credentials.json`, `.docker/config.json`, `appsettings.Production.json`, `user-secrets`); checks both file path and command text |
...
| `guard-bash-scope` | `Bash` | same resolution, but a missing scope does NOT fail closed — read-only roles legitimately run `git diff` and test commands; protected-path checks still apply to everyone |

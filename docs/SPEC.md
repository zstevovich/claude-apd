...
**v6.38 caveat — the `model` column is the ORCHESTRATOR's model.** Agent models are what a PROFILE sets and they are NOT in this row, so a profile question cannot be answered from this log. `apd report turns` reads the SERVED model out of each subagent transcript instead, and marks an agent that ran under more than one model in the window. Measured 2026-07-25: agents pinned to the bare alias `opus` were served `claude-opus-4-8` through 07-23 and `claude-opus-5` from 07-25 with no config change and no record — any turn/duration comparison spanning that boundary mixes two models under one label. `model-profiles.conf` therefore pins full model ids; a bare alias is a test failure.
...
### 7.4 Per-agent frontmatter hooks do not fire (v6.38)

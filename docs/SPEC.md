...
- `reconstruct-agents` — recovery for a CC **SubagentStop hook non-delivery** (harness bug, observed 2026-06-04: CC ran subagents but never fired SubagentStop, leaving `.agents` empty while transcripts exist). Rebuilds `.agents` from CC subagent **transcripts** at `${CLAUDE_CONFIG_DIR:-~/.claude}/projects/<slug>/<session>/subagents/agent-<id>.jsonl` (+`.meta.json` for `agentType`). Writes a start+stop pair ONLY for agent-ids whose transcript file exists (CC ground truth the orchestrator cannot fabricate) and whose completion is after `spec.done` (current task). Skips already-recorded ids. Loud `INFO|reconstruct-agents|...` audit entry. Integrity: a phantom agent with no transcript is never reconstructed, so genuine hook-failure is recoverable while fabrication stays blocked — the distinction `.agents`-fiat cannot make

...
| `run-role` / `rr` (v6.20) | `bin/core/run-role` | Prepare a git worktree for a **producer** role (prepare-only, Milestone C1). APD owns the worktree lifecycle — CC native `claude --worktree` is create-only and leaves stray dirs (measured), so run-role drives `git worktree add` itself: stable `<role>-work` branch at `.claude/worktrees/<role>-work`, reuse if already registered. Calls `worktree-bootstrap` (config in, **never** pipeline state), then runs the project's `.apd/dev-env-setup` if present (tracked, idempotent — a thin call to the project's own run script; APD doesn't know the stack). Prints `cd <path> && claude` by default. **`--launch` (C2, v6.21)** spawns CC in the worktree after prepare — it scrubs `APD_PROJECT_DIR` + `CLAUDE_PLUGIN_ROOT` then `exec claude` so the new session resolves the worktree as its OWN project root (R6), and degrades to prepare-only if no `claude` is on PATH (E11). Guards: E1 invoked inside a worktree → BLOCK exit 2; E2 operator role (`worktree=no`) → BLOCK exit 1; E4 stray dir → BLOCK exit 2 (**never auto-deletes** — global rule 1); E7 non-git → exit 1; E9 unknown role → exit 1; E3 active pipeline → warn; E6 dirty main → warn. `--list` shows producer roles. |
...
- **Operator roles** (`worktree=no`): devops (shared infra plane), debug (read-only diagnosis), master (integration/broker) — run from the main checkout; isolation doesn't help and would actively hurt (integrator must see shared state; devops manages the shared infra).

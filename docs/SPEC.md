...
FastMCP wrapper, runs via `uv run --with mcp python …`. **All MCP config — server registration AND per-tool approvals — ships in `plugins/apd/.mcp.json` since v5.0.10.** Codex 0.124+ auto-loads plugin-level `.mcp.json` (`core-plugins/loader.rs::normalize_plugin_mcp_server_value`), normalises `cwd` against the plugin root, and applies the resulting cwd to `Command::current_dir` (`rmcp-client/stdio_server_launcher.rs:231`). Our manifest sets `cwd: "../.."` so the resolved cwd is the repo root (parent of the plugin dir), where `mcp/apd_mcp_server.py` lives — see §17 for the `__file__`-based path resolution. Per-tool approval defaults live under `mcpServers.apd.tools.<name>.approval_mode = "approve"` in the same `.mcp.json` (Codex `RawMcpServerConfig` accepts `tools: HashMap<String, McpServerToolConfig>` per `mcp_types.rs:241`). `install-codex-config` writes nothing into `[mcp_servers.apd*]` since v5.0.10 — it only removes legacy blocks left by APD < v5.0.10, because per-tool blocks without a parent transport block crash Codex with `invalid transport in mcp_servers.apd`. 8 tools (full param details + security in §18).
...
| `SessionStart` | `bin/adapter/cdx/session-start` | 10s | Shortcut drift guard + `apd-init --quick` gap analysis (throttled 1h). TUI only — `codex exec` has no equivalent bootstrap path (see §14). |
...
| `.adversarial-pending` | reviewer phase | Full mode flag |
...
| `<project>/.codex/config.toml` | Project | install-codex-config (cleanup-only since v5.0.10) | Nothing under `[mcp_servers.apd*]` is written. Server + per-tool approvals both ship in `plugins/apd/.mcp.json`. Any pre-v5.0.10 APD blocks (top-level or per-tool) are stripped on next install — leaving them would either WARN ("plugin MCP overwrote earlier server definition") or crash Codex with `invalid transport`. |
...
- Source of truth: `VERSION` file at repo root + `version` field in plugin manifests
...
## 17. MCP server internals — `mcp/apd_mcp_server.py`

### 17.1 Module-level + bootstrap

- FastMCP wrapper: `mcp = FastMCP("apd")`. Functions decorated `@mcp.tool()` auto-exposed.
- 8 tools registered.
- No persistent daemon — Codex spawns fresh process per session via `uv run --with mcp python apd_mcp_server.py`.
...
## 18. Codex adapter scripts — `bin/adapter/cdx/`
...
1. **MCP cleanup-only (since v5.0.10)** — removes any pre-v5.0.10 APD MCP blocks from `<project>/.codex/config.toml`: top-level `[mcp_servers.apd]` (left by APD < v5.0.9) and/or per-tool `[mcp_servers.apd.tools.<name>]` (left by APD v5.0.9). Per-tool blocks without a parent transport block crash Codex with `invalid transport in mcp_servers.apd` — that bug is what motivated v5.0.10 in the first place. Both sets of blocks are now provided by `plugins/apd/.mcp.json`. The script writes nothing under `[mcp_servers.apd*]`; if there's nothing to clean, it's a no-op and config.toml is not even created. Backs up existing config.toml with timestamp suffix when there is a change. Exit code 10 signals changes.

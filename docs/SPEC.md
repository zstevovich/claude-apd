...
- `verifier` — runs `verify-all.sh`; blocks on failure; in Full mode blocks on missing `.adversarial-summary`; **supervision gate (v6.30)**: profile has supervisor row + Full → `_supervision_issue()` structural checks + dispatch-backed + stop-based churn cap + finality; `SUPERVISION_GATE_DEFAULT=warn`, env one-way (strict escalates, downgrades ignored+logged), no synthetic skip; writes verifier.done + `verified.timestamp`

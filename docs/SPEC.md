...
| `reload-done` / `rd` (v6.26) | `bin/core/reload-done` | Clear the `.apd/.pending-reload` marker after the user has run `/reload-plugins`. CC caches agent definitions at session start and `/reload-plugins` does NOT fire `SessionStart` (empirically tested), so APD can't observe a reload — `apd profile` drops the marker on a change and the SubagentStart drift-guard blocks dispatch while it stands. `reload-done` removes it (the no-restart path); a fresh `startup` session clears it automatically. Exit: 0 (cleared or nothing to clear). |
...
| `.pending-reload` *(in `.apd/`, NOT `pipeline/`)* | `pipeline-model-profile` writes, `track-agent` reads | **v6.26:** set when `apd profile` changes agents (`changed>0`); SubagentStart drift-guard hard-blocks dispatch while present (`APD_SKIP_RELOAD_GATE=1` overrides). Cleared by `apd reload-done` or a `startup` `session-start`. Lives OUTSIDE `pipeline/` so it survives `pipeline reset` — a needed reload outlasts any one task |

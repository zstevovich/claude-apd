...
# Extract agent metadata (available since Claude Code v2.1.72+)
...
if echo "$NORMALIZED_GIT" | grep -qiE "git commit"; then
  if echo "$COMMAND" | grep -qE "^APD_ORCHESTRATOR_COMMIT=1 "; then
    ...
    # Pipeline reset happens in PostToolUse hook (pipeline-post-commit.sh)

...
APD_CMD="$CLAUDE_DIR/scripts/apd-pipeline"
if [ ! -f "$APD_CMD" ] || ! grep -q "$APD_PLUGIN_ROOT" "$APD_CMD" 2>/dev/null; then
    mkdir -p "$CLAUDE_DIR/scripts"
    cat > "$APD_CMD" << SHORTCUT
#!/bin/bash
# APD Pipeline — shortcut to plugin's pipeline-advance.sh
# Auto-generated by session-start.sh
exec bash "$APD_PLUGIN_ROOT/scripts/pipeline-advance.sh" "\$@"
SHORTCUT
    chmod +x "$APD_CMD"
fi

# Create shortcut for pipeline-doctor
APD_DOCTOR="$CLAUDE_DIR/scripts/apd-doctor"
if [ ! -f "$APD_DOCTOR" ] || ! grep -q "$APD_PLUGIN_ROOT" "$APD_DOCTOR" 2>/dev/null; then
    cat > "$APD_DOCTOR" << SHORTCUT
#!/bin/bash
# APD Doctor — shortcut to plugin's pipeline-doctor.sh
# Auto-generated by session-start.sh
exec bash "$APD_PLUGIN_ROOT/scripts/pipeline-doctor.sh" "\$@"
SHORTCUT
    chmod +x "$APD_DOCTOR"
fi

echo "  Pipeline: bash .claude/scripts/apd-pipeline <command>"
echo "  Doctor:   bash .claude/scripts/apd-doctor"
echo ""

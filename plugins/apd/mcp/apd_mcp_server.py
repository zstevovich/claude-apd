...
def _bootstrap_shortcut() -> None:
    ...
    if shortcut.exists():
        try:
            ...
            # Upgrade pre-export shortcuts: require BOTH the plugin path and the
            # APD_RUNTIME tag, else rewrite (matches install-codex-config).
            if str(plugin_apd) in _content and "APD_RUNTIME=codex" in _content:
                return
        except OSError:
            pass
    shortcut.parent.mkdir(parents=True, exist_ok=True)
    # export APD_RUNTIME=codex — this shortcut is the Codex-side entry point, so
    # framework scripts (supervision honest-inert, metrics runtime/model columns,
    # builder/reviewer inline-review branches) correctly key runtime off it even
    # when a pipeline is advanced via the bare shortcut rather than the MCP tool.
    shortcut.write_text(
        "#!/bin/bash\n"

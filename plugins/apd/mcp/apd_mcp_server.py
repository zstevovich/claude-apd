...
def _bootstrap_shortcut() -> None:
    ...
    if shortcut.exists():
        try:
            _content = shortcut.read_text()
            # Already the version-agnostic launcher → leave it (matches install).
            if "_apd_home" in _content and "APD_RUNTIME=codex" in _content:
                return
        except OSError:
            pass
    shortcut.parent.mkdir(parents=True, exist_ok=True)
    # Version-agnostic shortcut: the plugin cache is versioned (<…>/apd/<X.Y.Z>/),
    # so pinning this path breaks on every plugin update (2026-07-14 ENOENT incident).
    # Resolve the current cache at runtime instead — mirror install-codex-config's
    # .codex/bin/apd launcher so the two never fight.
    baked = str(APD_PLUGIN_ROOT)
    parent = str(APD_PLUGIN_ROOT.parent)
    versioned = "1" if re.match(r"^\d+\.\d+\.\d+", APD_PLUGIN_ROOT.name) else "0"
    shortcut.write_text(
        "#!/bin/bash\n"
        "# APD version-agnostic shortcut — auto-generated by MCP server bootstrap.\n"
        "# Resolves the CURRENT plugin cache at runtime; do not hand-edit.\n"
        "export APD_RUNTIME=codex\n"
        f"APD_BAKED={shlex.quote(baked)}\n"
        f"APD_PARENT={shlex.quote(parent)}\n"
        f"APD_VERSIONED={versioned}\n"
        "_apd_home() {\n"

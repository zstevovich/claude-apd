...
- **`audit-drift` dimension C was one-way.** It asked "the framework has this marker, does the project?" and never the reverse, so a project carrying a section the framework had DELETED passed — the more dangerous half, because the orchestrator then follows retracted instructions as if they were current. When every marker is present, dim C now byte-compares against the shipped `rules/workflow.md` (a plain `cp` with zero placeholders, so byte-equality is the whole contract) and reports the changed-line count. Detection only: the automatic refresh stays marker-based, because a byte difference is frequently content `apd-setup` itself told the project to write, and the backup slot is first-wins — auto-overwriting would destroy every edit after the first, unbacked, and silently under Codex.

- **The permissions merge kept its own list of four sentinels.** If all four matched, the merge was skipped — so each time the required list grew, existing projects never received the new entries. It failed this way twice: the v6.10 deny expansion (patched in v6.16.1 by adding a deny sentinel) and again from the pre-v6.7 trio to seven pipeline files, which is why `.guide-marker`, `.adversarial-rationale.md` and both supervision files never arrived on Bambi — on exactly the paths APD promises to pre-approve so the gates it MANDATES do not prompt. The probe is gone; the merge computes the result and writes only when it differs. No second list to fall behind.

- **`/apd-setup` gained step 5c: reconcile the project 1:1, by asking.** Models against the declared profile, `workflow.md`/config/permissions via `audit-drift`, agent frontmatter against the shipped template, and fields the framework no longer ships. One report, one question, full alignment recommended — never a subset, because a half-aligned project then reports IN SYNC for a state nobody chose. Declining is a valid end state. Where references collide the authority is named rather than guessed: `model-profiles.conf` owns `model`/`effort` (a missing row means "this profile does not mention that role", never "reset to the default row", so a `cruise` builder is not dragged down to the template pin), the template owns structural fields, `scope` stays project-specific. `apd-init` is unchanged here by design — it reports who owns a model, not whether the value is right.

**The pre-release audit earned its keep for the fifth time, and the finding was mine.** Reserving `exit 1` for "nothing changed" collided with CPython, which exits 1 on any uncaught exception — and that call discards stderr. A truncated or conflict-marked `settings.json` therefore reported a green "permissions (pipeline + apd commands)" while applying nothing: v7.0.3 would have re-created the bug it was written to fix, on the projects that needed it, and called them healthy. No-change is `exit 3` now, 127 stays "python3 missing", and anything else warns with the command to diagnose it. The audit also caught two of the new checks passing on a deliberately broken mechanism (both greped a message instead of asserting state) and a 3-line window that guarded one of three `_profile_owns` branches — that one, once widened, immediately flagged a branch that turned out to be restoring a profile pin rather than overriding it.

Suite 1149 → 1167. Every mechanism has a mutation that flips exactly its own checks.
...
- **`.apd/lessons.md` — the pipeline now keeps the class, not just the fix.** APD enforced the *finding* and dropped the *generalization*. An accepted adversarial finding produced a fix in code and nothing else: the class it belonged to left with the run, and the next task met the same shape from scratch. `adversarial-rationale-archive.md` has accumulated every finding and disposition since v6.13 and nothing ever read it back — the framework was producing knowledge and discarding it.
  ...
  `apd pipeline lesson "<rule as a class>" "<what it cost>"` appends to `.apd/lessons.md`: runtime-neutral (one path on CC and Codex), outside `pipeline/` so no reset wipes it, and trackable in git because it is team knowledge rather than session state. A mandatory source keeps each line traceable to something the project actually paid for; a 20-character floor on both fields keeps out rules nobody who was absent could apply; an exact-duplicate rule is refused, because a file that repeats itself is the noise this must not become. Read back with `apd pipeline show lessons`, logged as `INFO|lesson-recorded`.

  **Deliberately not auto-distilled from the archive.** A machine summary of every past finding becomes noise that every builder then carries in its system prompt, and noise there is worse than silence — so it takes a decision, at the one moment the decision is cheap. `/apd-finish` now asks, in both mirrors, whether an accepted finding generalizes, and warns past ~20 entries that a file gets skimmed rather than read.

  **The half that makes it education rather than an archive:** the builder charter tells builders to read `.apd/lessons.md` before they start, and it is step 1 of the builder workflow. A recorded class then arrives automatically on every future dispatch instead of waiting for someone to go looking. §117 asserts both halves — the command's validation and durability across a reset, and the fact that something routes the file to a builder at all.

- **A builder charter — the role that writes the code was the only one nobody taught.** Guards are mechanics; the average comes from education, and APD had that backwards by role. The orchestrator gets a mandatory operating manual it cannot advance without. The supervisor gets a four-question charter. The adversarial reviewer gets a template that explains its own blindness. The agent that actually produces the code got a FORBIDDEN list and a scope path — the least-educated role and the most consequential one.

  The charter carries only what nothing else tells a builder. **Who reads the diff after it stops:** a contextual reviewer, an adversarial reviewer that *cannot see the spec, the plan or any statement of intent*, and a supervisor judging the final post-fix diff. That one fact changes behaviour — code defensible only once you know the intent will draw findings, and "the spec asked for it" is not available as an answer to a reader who cannot see the spec. **Where the bar rises:** two or more of irreversible side effect / atomicity across an unguaranteed boundary / concurrency over shared state / state accumulating over time, because there a defect lives in the composition of the steps rather than at a line you can read — so an executed proof replaces an opinion, a test that FAILS before the change and passes after, one per error branch, written in the same step as the fix. **What the spec puts on it:** the regression surface with its `**Cover:**` items is the builder's obligation, not the reviewer's. **That scope is a boundary:** if the work needs a file outside it, say so and stop — routing the same write through a shell command is the move the guard exists to catch. **And that a repeated shape is reported, not fixed** — quietly widening the diff breaks the scope and the reviewer's baseline at once.

  Builder agents are project-authored (their scope is the project's), so `apd-init` cannot regenerate them from the template. It inserts the block once, keyed on the machine marker `<!-- apd:builder-charter -->` rather than on prose, into agents that can WRITE only — the read-only roles carry their own contracts and would be diluted by this one. Existing projects self-heal on the next session-start. §116 proves each half by its own mutation: dropping the insert flips three checks, and dropping the writable-only filter flips exactly the one asserting read-only agents are left alone — the assertion that is vacuous under the first mutation and only discriminates under the second.

- **`guard-spec-blind` — the adversarial reviewer's blindness is now enforced, not merely intended.** The role's whole value is positional: it judges the diff without knowing the intent behind it. That was a claim. The template never mentions the spec, but the agent carries `Read, Glob, Grep, Bash`, its `Read` hook is `guard-secrets` (env files and keys) and `guard-pipeline-state` is `Write|Edit`. Measured on a fixture, every channel returned exit 0 for `cat .apd/pipeline/spec-card.md` — and reading the spec is the most natural thing an agent does when it wants to understand a task, so this was default behaviour, not a hypothetical attack.

  The guard blinds the whole `.apd/pipeline/` directory and the APD memory directory — a previous pass's rationale contaminates a fresh look as much as the spec does, and `adversarial-rationale-archive.md` carries every earlier run's triage. It also closes the sanctioned read paths (`apd pipeline show`, `apd report`), which print the same content through a different door. **One exception:** `.apd/pipeline/.reviewed-files` stays readable — it IS the review scope and the template instructs the agent to read it; blinding it would break the role the guard exists to protect.

  Two channels, because one matcher cannot cover both: a path matcher has nothing to match in a Bash command string. The path channel canonicalizes (so a traversal onto the spec is caught) and fails closed on anything it cannot resolve; the Bash channel scans tokens with globbing disabled, so `cat .apd/pipeline/*` is caught even though no blinded filename appears in it.

  **Caller identity comes from the payload.** The obvious home for a per-role guard is the agent's own frontmatter `hooks:` block — but that block never executes (see the next entry), so mounting it there would have been decoration. The guard runs session-level for every caller and exits immediately unless the payload's `agent_type` is the adversarial reviewer. The orchestrator (which has no `agent_type`), the builders, the reviewer and the supervisor all pass through — blinding any of them would deadlock the pipeline. Opt-out `APD_SKIP_SPEC_BLIND_GUARD=1`. **CC-only, inert under Codex** — the Codex payload carries no equivalent role tag on tool calls, and a blanket hook would blind the orchestrator; declaring that plainly follows the v6.33 supervision precedent rather than shipping a false claim.

  Installed projects self-heal: `apd-init` treats a missing `guard-spec-blind` as a stale-agent content marker and refreshes from template — preserving a profile-managed `model:`/`effort:`, since a blind refresh would have silently reverted a `burn` or `eco` project to the template default (the v6.16.1 no-revert contract, which every new marker re-breaks).

- **Agent write-scope and the secrets guard are enforced again — they had never run on CC.** Building the guard above surfaced the reason its first wiring would have been inert: **per-agent frontmatter `hooks:` blocks do not fire.** Measured on CC 2.1.220 — a logging hook declared there never ran, in either schema form, while the identical hook in `settings.json` fired and blocked; the hook script works standalone and the subagent genuinely ran, so neither is the explanation.

  `guard-scope` (carrying each agent's `{{SCOPE_PATHS}}`) and `guard-secrets` appear nowhere in `hooks/hooks.json` — frontmatter was their only wiring. So on CC, **an agent's write scope was never enforced and the secrets guard never executed**. `guard-bash-scope` was wired session-level but without scope paths, so it enforced the protected paths (pipeline state, plugin cache) and nothing agent-specific. Codex was the better-protected runtime throughout: its writes go through `apd_guard_write` → `bin/core/guard-scope`, the path hardened in v6.37.0.

  Fixed by moving enforcement to `hooks/hooks.json` and identifying the caller from the payload, which carries `agent_type` and `agent_id` for tool calls made inside a subagent and neither for the orchestrator's own. New `bin/lib/agent-scope.sh` resolves the declared scope from the agent REGISTRY — YAML `scope:` first, then the `guard-scope` hook command — never from caller arguments, so an agent cannot widen its own scope (the rule `apd_guard_write` already applied). A readonly agent blocks outright; a writable agent with no declared scope blocks fail-closed; an agent outside the APD registry is left alone, because users run their own subagents in APD projects. The bash side deliberately does NOT fail closed on a missing scope — read-only roles legitimately run `git diff`.

  The frontmatter hook command keeps its role as the scope DECLARATION; only enforcement moved.

  **This is a real behaviour change, not a refactor:** projects will now see out-of-scope writes blocked where they silently succeeded before. That restores the floor rather than raising it.

  Live-verified through real CC with the discriminating pair, from inside a subagent: an in-scope write succeeded, an out-of-scope write was blocked (`BLOCKED: File outside/evil.txt is outside the allowed scope. Allowed paths: src/`). The pair matters — the first attempt blocked the out-of-scope write for the wrong reason (the resolver died under `set -e` on a grep that simply found nothing, yielding an empty scope and blocking everything), and only an in-scope ALLOW could tell the two apart.

  Tests: §111 spec-blind (34 checks) + §112 the restored layer (20), **983 → 1038**, exit codes rather than messages, ALLOW directions covered, and the resolver exercised under the same `set -euo pipefail` its callers use. Mutation-proven: reintroducing the `set -e` bug flips 3 checks, removing the `hooks.json` wiring flips the reachability check, disabling fail-closed flips 1, and skipping the `agent_type` resolution flips all 4 blocking checks.
...
- **The portability guard is symmetric, and CI finally runs on Linux.** `guard-bash-portability` shipped Darwin-only: it protected macOS users from GNU-isms and was inert on Linux. That is the wrong way round for a framework developed on macOS — a BSD-ism reaches a Linux user unchallenged, while the reverse is caught the moment it is typed. The Linux direction now blocks `sed -i ""`, `stat -f`, `date -v`/`-j`/`-r <epoch>`, `tail -r`, `md5`, `base64 -D`, `sysctl hw.*` and `find -E`, each naming the GNU equivalent.

  The two that matter most do not fail loudly, they lie: on GNU `sed -i ""` consumes the empty string as the SCRIPT, so the command exits 0 and the file is unchanged; `stat -f` means `--file-system`, so it exits 0 and returns the wrong data. Everything else at least errors.

  The framework's own 77 scripts were audited in both directions and came out clean — every `stat`/`date` call already carries a BSD-first, GNU-fallback chain, and the one `tac` vs `tail -r` spot is explicitly commented. But **nothing had ever executed them on Linux**: there was no CI at all, while `CLAUDE.md` listed "CI | GitHub Actions" in the stack table. `.github/workflows/test.yml` now runs the full suite on `macos-latest` and `ubuntu-latest` (`fail-fast: false` — when one platform breaks, the other's result is the more interesting half), plus a job that drives BOTH guard directions from BOTH hosts, since a host only proves its own side natively. The `CLAUDE.md` row now says what actually runs.

  Caveat stated rather than discovered: the suite has never executed on Linux, so the first workflow run is the verification, not a formality.

  Tests: 9 BSD-isms blocked, 9 GNU forms passed, and one symmetry check — `sed -i ""` must be ALLOWED on darwin and BLOCKED on linux, since a check that looks at one side cannot tell a symmetric guard from one that is simply strict everywhere. The block first shipped green against a torn-down fixture (`APD_PROJECT_DIR` pointing at a deleted directory makes the guard resolve `APD_ACTIVE=false` and exit 0, so every assertion passed against nothing); it now owns its fixture and opens with a liveness check that fails if the guard is inert.

### Fixed

- **Two `apd-audit` checks pushed projects backwards.** The skill that exists to catch drift carried drift, and twice it was drift with a direction: it required builders to be `sonnet` and the reviewer `opus`, so auditing a correctly pinned project would have walked it back to the bare aliases this release removed everywhere else; and it demanded a `Write(...)` permission twin alongside `Edit(...)`, which CC 2.1.208 ignores and `apd-init` strips on every run — audit and init would have undone each other on each session-start. It also flagged `memory: project` as universally required, inverting the `memory: none` contract that makes the adversarial reviewer and the supervisor worth dispatching, named `bin/core/` for hook paths that live under `bin/adapter/cc/`, listed four workflow markers where the script checks six, pointed the mandatory-skills table at brainstorm rather than the guide, and never checked that the adversarial and supervisor agents exist at all — omitting the file whose absence disabled the layer.

- **`apd-toggle` walked the user into a block it never mentioned.** Its description invites use when someone "hit a guard block they want to bypass"; since v6.29 that exact move exits 2 with `toggle-off-active-pipeline`, and the sanctioned routes — `spinoff-finding`, `raise-cap`, `reset` — were named nowhere in the skill.

- **`apd-finish` told CC to push to `origin`.** v6.33 shipped `git-remote-resolve` precisely so the remote is never assumed, and updated the Codex mirror; the CC mirror kept three hardcoded `git push -u origin` lines. `apd-profile` still described supervision as an eco-only layer and did not state that a profile owns only the roles it names. `apd-brainstorm` — in all three of its mirrors — taught a free-form `Regression surface:` while `verify-regression-surface` parses `- RS<N>:` + `**Cover:**`, so a spec the skill was satisfied with failed at the builder advance: producer and gate agreed on the content and disagreed on the shape.

- **The adversarial layer went off in silence when its agent definition was missing.** The reviewer step wrote `.adversarial-pending` only when an `adversarial-reviewer` definition existed, and every verifier adversarial check keys on that marker. So with no definition there was no marker, no requirement, and no complaint: measured as a discriminating pair — with the definition present the verifier BLOCKED on the missing pass; with it absent the same pipeline created `verifier.done` carrying zero findings, no rationale, no BLOCK and no audit entry.

  Two things made it reachable rather than theoretical. `.claude/agents/` is covered by no guard, so `rm` was a one-command, unlogged bypass of the floor. And a project sits in exactly that state after a first `apd-init`, which scaffolds no agents — the second run does, which is why `/apd-setup` (it runs both scripts) never exposed it. Three fixtures in the suite were themselves relying on the hole to advance their reviewer, which is the measure of how invisible it was.

  The requirement no longer depends on the file existing. A missing definition is a setup fault, not an opt-out: the reviewer advance BLOCKs with `adversarial-agent-missing`, decided BEFORE `create_done` so a block does not leave the phase half-advanced. The verifier asks the same question again (`adversarial-unaccounted`), because the definition can be removed after the reviewer signs off and the reviewer-side check cannot see that. `adversarial: skip` at ≤2 R-criteria remains the only sanctioned way past, now evaluated by one shared `_adversarial_opt_out_ok()` rather than duplicated. §115 covers both sites and — the part that matters — includes the negative controls: a valid Lean opt-out must still advance and still reach `verifier.done`, or "closes the hole" would be indistinguishable from "blocks every small task". Red-green: neutering the reviewer check flips exactly its two assertions, the verifier check exactly its two.

  `pipeline-doctor` was the one diagnostic that could have caught this, and it stated the inverse — `warn "No adversarial-reviewer agent — Full pipeline adversarial dispatch will BLOCK"` while nothing blocked at all. It now fails, with a message that matches the measured behaviour.

- **`apd-setup` generated project agents with bare model aliases.** `reference/agent-templates.md` specified `model | sonnet` for builders and `model | opus` for the reviewer, and its generation rule says to build agents from that table rather than copy the templates — so the aliases won over the pinned template ids. This release pinned every model id across the conf, the shipped templates and `apd-init` repair, and guarded all three with tests; the sweep missed the generator that writes agents into real projects, which is the path that mattered most. Both are now full ids. The same file also named the wrong hook prefix (`bin/core/` instead of the `bin/adapter/cc/` shims) and did not say that the hook block is DATA — per-agent hooks never fire, but `agent-scope.sh` reads the `guard-scope` command out of that block to resolve an agent's writable scope, so a wrong SCOPE_PATHS list there is a real enforcement defect rather than a cosmetic one.

- **`apd-init` pre-approved three of the seven pipeline files the framework mandates writing.** The `settings.json` allow list carried `spec-card.md`, `implementation-plan.md` and `.adversarial-summary` — the pre-v6.7 set — while `guard-pipeline-state` has long allowed `.adversarial-rationale.md`, `.guide-marker` and the two supervision files. Those are not optional extras: the rationale is hard-gated at the verifier and `.guide-marker` is hard-gated at the spec advance, so the framework required writes it had never pre-approved. Now all seven, Edit-only.

- **A declared model profile was treated as ownership of every role.** `apd-init` skipped its model check whenever `MODEL_PROFILE` was set, on the assumption that `apd profile` owned the value. Since this release removed the `default` fallback for named roles, a conf with no `reviewer` row leaves `code-reviewer` alone — so init deferred to the profile, the profile ignored the role, and nothing checked it. Init still does not rewrite (reverting a hand-picked model on every session-start is the footgun v6.16.1 removed) but now says which case it is: managed by the profile, on the template pin, or unowned and drifted. `apd-init` also announced the supervisor as "required by eco profile", one release after every profile gained a supervisor row, and `reference/init-checklist.md` listed neither the adversarial nor the supervisor agent among the artifacts its gap analysis checks — omitting the very file whose absence disabled the layer.

- **The pipeline guide had drifted from the framework it documents, and a stale contract directs with full confidence.** `apd-pipeline-guide` is what the orchestrator reads before every task; three of its claims were measurably wrong against the code, on a suite that was green at 1057/0.

  It recommended raising a blocked cycle cap by editing `max_cycles` in the signed spec — three releases after `raise-cap` shipped BECAUSE that path forces a spec re-advance which wipes `.agents` and destroys the builder evidence already earned. The gate's own BLOCK message named `raise-cap`; the guide, and `workflow.md`, still pointed at the destructive path, so the only thing teaching the safe recovery was the error text itself. It claimed `pipeline_mode: polish` "skips adversarial for 1-2 R hotfixes" — polish only lowers the caps 2 → 1; the opt-out is a separate field (`adversarial: skip — <reason>`, honoured only at ≤2 R-criteria) that the guide never named, so an orchestrator following it planned a Lean run and met a Full-run gate with no way to recover. And it scoped supervision to `eco`, which this same release invalidated by giving every profile a `supervisor` row — the drift was introduced by the commit next to it. That last one also widened the strict-flip blast radius from eco projects to every CC project, which the rollout notes now say.

  The Codex mirror carried its own: it still described the v6.30 supervision gate as profile-inert and told the orchestrator to arrange a pass from the CC side, obsolete since v6.33 made the gate honest-inert on the Codex runtime; and it never mentioned `apd_prepare_dispatch`, though v6.36 removed the inline builder/reviewer path and the gates now require a real native subagent.

  Beyond the wrong claims, the guide was silent on mechanisms that stop a run: `orchestrator-code-write` (the orchestrator may not write production code — the pipeline's premise, absent from its operating manual), `send-message-during-pipeline`, the portability guard, the rationale accounting BLOCKs (it taught the file's shape but not that T/A/D must reconcile with the summary), and `adversarial-summary-without-dispatch` — while documenting its supervision twin. Three more became reachable on CC for the first time in this release: `spec-blind`, `secret-access` and `out-of-scope-write` were wired to per-agent frontmatter hooks, which never fire.

  Both mirrors, `workflow.md`, `AGENTS.md` and `SPEC.md` are corrected, and the BLOCK tables are split into gate BLOCKs (fire at an advance) and guard BLOCKs (fire on a tool call), which is the architecture rather than one flat list.

- **§114 makes this class of drift mechanical.** Docs are asserted against what the code emits, not against a remembered version: every `log_block` reason in `pipeline-advance` must appear in the guide (add a gate BLOCK and the suite fails until it is documented or explicitly exempted); every guard wired in `hooks.json` must be named there; and the supervision claim is checked against `model-profiles.conf` itself, so a topology change fails the docs rather than silently outdating them. Red-green: with the docs reverted, 16 of the section's 18 checks fail, each naming its own defect — the two that stay green are the ones that read code rather than prose.
...
**Mechanism.** A new MCP tool `apd_prepare_dispatch(apd_role)` reserves the next pipeline phase for exactly one native Codex subagent and returns a Codex-safe `task_name` (Codex reports `agent_type=default`, so it cannot select a custom role directly). A new lifecycle hook `bin/adapter/cdx/track-subagent` consumes that short-lived binding at `SubagentStart`, binds the real runtime `agent_id` to the canonical APD role, and closes the pair at `SubagentStop` — writing the `.agents` ledger through `track-agent`. `apd_prepare_dispatch` is role→phase gated (builder must be writable; reviewer/adversarial read-only), path/symlink-validated against the agents root, and serialized via a single 120s-TTL pending binding. `install-codex-config` wires the two hooks (matcher `*`, idempotent merge that preserves unrelated user hooks and replaces stale entries); `codex-doctor` verifies the wiring and treats `.apd/agents/` as the canonical role registry (the old TOML-migration nudge is gone); `apd_adversarial_pass` now requires a real `|start|adversarial-reviewer|` event, and the verifier gate additionally requires the paired stop. `AGENTS.md` + the Codex TDD/debug/finish rules + `apd-tdd`/`apd-finish` skills are rewritten so the orchestrator coordinates and dispatches rather than implementing/reviewing source itself.

### Audit

Independent pre-commit framework-audit (decontextualized agent, rule 3) — CLEAN, no CRITICAL, no floor regression: the change materially *strengthens* the floor (Codex self-attestation is gone; no fabrication/bypass path found — `.agents` is blanket-blocked from Write/Edit and bash on Codex, and the compiled validator rejects zero-duration forged entries). It found **1 IMPORTANT + 3 MINOR, all fail-closed**. Fixed this release:
- **IMPORTANT — adversarial gate false-block on hook non-delivery.** The tightened gate requires both `|start|` and `|stop|` for `adversarial-reviewer`. When a `SubagentStop` hook silently drops (a documented CC failure mode, observed 2026-06-04 / 2026-06-29) a *completed* adversarial pass was blocked with a misleading "agent was never dispatched" message that hardcoded CC syntax and offered no recovery. Now the gate distinguishes start-without-stop (hook non-delivery → points at `reconstruct-agents` on CC, or "wait for the child to stop" on Codex) from no-start-at-all (runtime-appropriate dispatch hint — `apd_prepare_dispatch` on Codex, `Agent({...})` on CC), mirroring the builder/reviewer gate treatment. Red-green verified.
- **MINOR — malformed dispatch binding wedged SubagentStart.** Under `set -euo pipefail`, a `jq` failure on a corrupt `.dispatch-pending.json` aborted `track-subagent` before its own cleanup, so every subsequent `SubagentStart` hit the same bad file. A `jq` failure is now treated as an empty role → the bad binding is removed.

Deferred (fail-closed, low-reach): `_agents_dir` vs `APD_AGENTS_DIR` divergence on a non-standard `.codex/agents`-only layout (the doctors already `fail` on it); pending-binding attribution imprecision inherent to Codex exposing only `agent_type=default`.

### Tests

`test-codex-adapter` — §101 (native lifecycle: prepare-dispatch validation + safe `task_name`, real-UUID→role pairing round-trip, hook wiring, no stale inline bypass) plus §102 (adversarial gate hook-non-delivery messaging, CC + Codex) and existing sections reseeded with real Codex start/stop pairs → **879 → 901/0**. SPEC.md reconciled for the removed inline model (tool count 9→10, `.agents` telemetry source, lifecycle builder step, Codex hooks table, reconstruct scope).
...
The supervisor is NOT a second adversarial. Its charter is four questions about the FINAL state: (1) does the final diff still satisfy every R-criterion, (2) did fix-of-findings introduce collateral, (3) do the Regression-surface/Evidence claims hold against the diff, (4) is this safe to commit. Bug-hunting the new code is explicitly forbidden — that already happened, decontextualized, in the adversarial pass (whose positional value is untouched). The supervisor stacks BOTH independence axes over eco builders: fresh context AND a different, stronger model.

- **Profile coupling as data:** a `supervisor` role row in `model-profiles.conf` — presence of the row = supervision applies. v1 ships it on `eco` only (`eco|supervisor|claude-fable-5|max`); `cruise`/`burn` have no row → gate inert. The `supervisor` role has NO default fallback: on profiles without a row the agent file is left untouched (keeps its frontier template pin instead of being dragged to builder tier).
- **Gate at the verifier (adversarial pattern 1:1):** when the declared `MODEL_PROFILE` carries a supervisor row AND adversarial ran (Full mode — Lean stays lean), the verifier checks `.supervision-summary` (`SUPERVISION:T:A:D` + Notes) and, for `T>0`, `.supervision-rationale.md` with the SAME per-finding contract as the adversarial rationale (Severity/Status/Rationale, three dispositions including spinoff, ≥40-char dismissals, 100%-orchestrator-dismiss block). Plus: a **dispatch-backed check** (summary without a `supervisor` start in `.agents` = fabrication), a **finality check** (the last supervisor STOP must be the last agent activity on record — a pre-fix supervision cannot catch fix-of-findings collateral, which is the point of the layer), and a **churn cap** (2 completed passes; exhausted dispatches don't count).
- **No opt-out, by design:** no spec-card field, no skip flag, no `--force`. The only ways out are `apd pipeline reset` or switching profile BEFORE the spec advance (mid-pipeline profile switch is already refused). **Env is one-way:** `APD_SUPERVISION_GATE=strict` may escalate above the default; any downgrade attempt is ignored and audited (`supervision-gate-downgrade-ignored`). There is no `off` and no synthetic skip — the independent pre-commit audit showed a gate an env prefix can silence is cheaper to bypass than to satisfy.
- **Staged rollout:** `SUPERVISION_GATE_DEFAULT=warn` (Phase 1 — advisory, collects live evidence; warn fires are audited as `supervision-warn`, passes as `supervision-pass|model=…` for per-run attribution). Phase 2 flips the default to strict after eco+supervision runs are compared against the cruise baseline — at that flip, verify-apd Section 8 MUST pre-write supervision fixtures for eco projects (recorded in SPEC §24).
- **Supervisor agent:** new `templates/supervisor-template.md` (`memory: none`, `model: claude-fable-5`, read-only toolset); scaffolded by apd-init as the fourth default agent, with no hardcoded model repair (profile owns rewrites).
- **State lifecycle:** both files wiped at all four sites (spec re-advance, reset, both rollback branches) and archived into `adversarial-rationale-archive.md` (own `### Supervision rationale` sub-block) at the task-terminal wipes; `show state` digest line; `spinoff-finding` precondition widened to supervision-on-record; metrics columns 16–18 (`sup_t|sup_a|sup_d`).
- **Hybrid CC+Codex:** projects sharing the APD config see the gate in Codex sessions too — the Codex `guard-file-edit` allowlist carries both supervision files and the Codex guide documents the case (run the supervision pass from the CC side). Pure Codex never declares `MODEL_PROFILE` → inert.

### Fixed

- **`apd report` (pre-existing, v6.7.1):** the last-run metrics parser read 12 named columns, so columns 13+ were absorbed into `agents_x` on 15-col rows — silently suppressing the MaxTurn-exhaust line. Every column is now named with a `_rest` sink (same fix applied to the `apd pipeline metrics` aggregate reader, where the absorption made `$((sum + adv_w))` evaluate `|` as a bitwise OR: warns 2 → 6).

### Audit

Independent pre-commit audit (decontextualized agent) + a verification pass: **no floor regression in existing gates**; 6 IMPORTANT findings all fixed in-session (two-way env override contradicting the no-opt-out decision → one-way; the bitwise-OR metrics bug; the hybrid Codex dead-end; the missing finality check; starts-based cap burning budget on maxTurn exhausts; zero warn-path telemetry). Full record in `docs/plans/supervision-layer.md`.

### Tests

- **test(test-codex-adapter §95):** +21 net — 8 static (conf rows, template contract, gate wiring incl. one-way env + no `off`, 4 wipe sites + archive, guards + education surfaces, init scaffold, spinoff precondition, metrics/report columns) + 14 live (warn default, strict BLOCK + audit, valid dispatch-backed pass, wipe + archive on re-advance, fabrication BLOCK, churn-cap BLOCK, no-profile inert, cruise inert, synthetic-still-active + SYNTHETIC-tagged telemetry, downgrade-ignored, finality BLOCK, 18-col metrics no-corruption, profile apply no-fallback/eco-pin) + 3 drift fixtures + the 15→18-col e2e lock-in. **826 / 0.**

**Migration:** none for cruise/burn/Codex (gate inert). eco projects: next `apd profile eco` apply pins the supervisor agent; the verifier emits an advisory WARN until a supervision pass is recorded — a hard BLOCK only arrives with the Phase 2 strict flip, announced in a future release.
